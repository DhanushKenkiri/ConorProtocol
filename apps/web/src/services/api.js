...
export const getUserAgreements = async (address) => {
  try {
  ...
  } catch (error) {    // Check if it's a network connection error or timeout
    ...
    // Return object with empty array instead of throwing to prevent UI breaks
    return { 
      agreements: [],
      error: error.message
    };
 ...
 * @param {string} agreementData.value - Agreement value in ETH
 * @returns {Promise<Object>} - Response with agreement address and transaction hash
 */
